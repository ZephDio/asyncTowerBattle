...
import { BattleSoldier } from "../../../units/battle/implementation/soldier-battle";
...
export class SoldierBattleBarrack implements BattleBarrack<SoldierRecruit> {
  ...
  produce() {
    this.addRecruit(
      new BattleSoldier(this.recruit, { x: this.position.x, y: this.position.y }, this.path, this.targetCastle, this.removeRecruit),
      this
    );
  }
}
